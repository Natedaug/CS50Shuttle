...
function chart()
{
    ...
    for (var i = 0; i < shuttle.seats.length; i++)
    {
        ...
        {
            html += "<li>" + shuttle.seats[i].name + " of " + shuttle.seats[i].house + "</li>";
        }
    }
    html += "</ol>";
    $("#chart").html(html);
...
function dropoff()
{
    $("#announcements").html("It's not safe this far out!!");
    for(var i in HOUSES)
    {
        if(shuttle.distance(HOUSES[i].lat, HOUSES[i].lng) <= 30)
        {
            for(var j = 0; j < shuttle.seats.length; j++)
            {
                if( shuttle.seats[j] != null && i == shuttle.seats[j].house )
                {
                    $("#announcements").html("Buuhhh-Bye!");
...
function keystroke(event, state)
{
    ...
    $("#announcements").html("Chug-a-luga Chug-a-luga");
    //q or Q
    if (event.keyCode == 81 || event.keyCode == 113)
    {
        if(shuttle.velocity<200)
            shuttle.velocity += 20;
        return false;
    }
    //z or Z
...
function pickup()
{
    alert(shuttle.tiltAngle);
    $("#announcements").html("Get Closer!!");
    ...
    { 
        ...
        {   
            var fresh = true;
            for(var j in HOUSES)
                if( j == PASSENGERS[i].house)
                    fresh = false;
            var nes = nextEmptySeat();
            if(nes != -1 && !fresh)
            {
                shuttle.seats[nes] = PASSENGERS[i];
                features.removeChild(PASSENGERS[i].placemark);
                PASSENGERS[i].marker.setMap(null);
                $("#announcements").html("Got 'em!");
                chart();
            }
            else if(fresh)
                $("#announcements").html("No freshmen allowed!!");
            else
                $("#announcements").html("No seat for you!!"); 
        }
            
    }
}

//return next empty seat or -1 if no seats are available
function nextEmptySeat()
{
    var i = shuttle.seats.length - 1;
    while(i > -1 && shuttle.seats[i] != null)
        i--;
    return i;
}
...
function populate()
{
    ...
    for (var i = 0; i < PASSENGERS.length; i++)
    {
        ...
        }while( dist2(HOUSES[PASSENGERS[i].house].lat, HOUSES[PASSENGERS[i].house].lng, building.lat, building.lng )<200);
        ...
        //remembers passenger's placemark and marker for pick-up's sake
...
function dist2(lat, lng, lat2, lng2)
{
    var R = 6371;
